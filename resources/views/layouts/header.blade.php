  ...
  <header class="drop-shadow-lg z-20">
    ...
    <div @click.away="open = false" class="relative" x-data="{ open: false }">
        <button @click="open = !open" class="flex flex-row items-center w-full px-2 py-2 mt-2 text-sm font-bold text-left bg-transparent rounded-lg  style-menu-navbar md:w-auto md:inline md:mt-0 md:ml-3 focus:outline-none focus:shadow-outline">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute z-10 right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
            <div class="px-2 py-2 bg-background_light rounded-md shadow ">
                @foreach ($menu->submenu as $submenu)
                    <div class="relative">
                        <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="{{ $submenu->url }}">{{ $submenu->nama_submenu }}</a>
                        @if ($submenu->submenus !== null && $submenu->submenus->isNotEmpty())
                        @if ($submenu->submenus->isNotEmpty())
                            <div x-data="{ open: false }">
                                <button @click="open = !open" class="absolute right-0 top-0 mt-2 mr-4 text-gray-400 hover:text-gray-900 focus:outline-none">
                                    <svg fill="currentColor" viewBox="0 0 20 20" :class="{ 'rotate-90': open, 'rotate-0': !open }" class="inline w-4 h-4 transition-transform duration-200 transform">
                                        <path fill-rule="evenodd" d="M6 9a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm0-3a1 1 0 100-2h6a1 1 0 100-2H6a3 3 0 00-3 3 1 1 0 001 1zm8.293 2.293a1 1 0 011.414 1.414l-2 2a1 1 0 01-1.414 0l-2-2a1 1 0 011.414-1.414L12 9.586l2.293-2.293z" clip-rule="evenodd"></path>
                                    </svg>
                                </button>
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute left-0 z-20 mt-2 w-48 origin-top-left rounded-md shadow-lg">
                                    <div class="px-2 py-2 bg-background_light rounded-md shadow">
                                        @foreach ($submenu->submenus as $sub_submenu)
                                            <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="{{ $sub_submenu->url }}">{{ $sub_submenu->nama_submenu }}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
