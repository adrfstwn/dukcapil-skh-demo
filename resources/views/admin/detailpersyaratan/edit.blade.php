...
@section('content')
    <section id="detailpersyaratan-edit">
        <div class="container">
            <div class="flex flex-col p-6 border-[1.5px] border-white shadow-md bg-slate-50 rounded-2xl shadow-gray-200">
                <div class="flex flex-col">
                    <h2 class="text-2xl font-bold md:text-3xl font-monserrats text-primary_teks">Edit Detail Persyaratan Section</h2>
                    <p class="text-base text-secondary_teks">Input for detail persyaratan section</p>
                </div>
                <form action="{{ route('detailpersyaratan.update', ['id' => $detailpersyaratan->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="my-2">
                        <label for="nama-input" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Nama</label>
                        <input type="text" name="nama" value="{{ $detailpersyaratan->nama }}" id="nama-input"
