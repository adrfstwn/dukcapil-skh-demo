        ...
        <div class="container">
            <div class="flex flex-col p-6 border-[1.5px] border-white shadow-md backdrop-blur-md bg-slate-50 rounded-2xl shadow-gray-200">
                <div class="flex flex-col">
                    <h2 class="text-2xl font-bold md:text-3xl font-monserrats text-primary_teks">Edit Download</h2>
                    <p class="text-base text-secondary_teks">Edit download section</p>
                </div>
                <form action="{{ route('download.update', $download->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="my-2">
                        <label for="kategori_id" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Kategori Download</label>
                        <select name="kategori_id" id="kategori_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary dark:focus:border-primary">
                            @foreach ($kategoriDownloads as $kategori)
                                <option value="{{ $kategori->id }}" {{ $download->kategori_id == $kategori->id ? 'selected' : '' }}>
                    ...
                    <div class="my-2">
                        ...
                        <input type="text" id="judul" name="judul" value="{{ $download->judul }}"
                    ...
                    <div class="my-2">
                        ...
                        <textarea id="deskripsi_download" name="deskripsi_download" rows="4"
                            ...
                            placeholder="Write your description here...">{{ $download->deskripsi_download }}</textarea>
                    </div>
                    <div class="my-2">
