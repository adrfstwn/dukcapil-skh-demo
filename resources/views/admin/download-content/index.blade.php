...
<section id="Download">
    <div class="container">
        <div class="my-6">
            <button type="button" onclick="window.location.href='{{ route('download.create') }}'"
                        ...
                        <div class="mb-3 flex flex-col gap-3 justify-between">
                            ...
                            <div class="flex flex-col gap-6">
                                <p class="text-base font-medium font-nunito text-primary_teks">
                                    Kategori : <span
                                        class="text-sm text-primary font-bold">{{ $download->kategori->nama_kategori }}</span>
                                </p>
                                <p class="text-base font-medium font-nunito text-primary_teks">
                                    Status : <span class="text-sm text-primary font-bold">{{ $download->status }}</span>
                                </p>
                            </div>
                            <div class="group mt-4 inline-flex flex-wrap items-center gap-3">
                                @if ($download->file)
                                    <a href="{{ asset('storage/' . $download->file) }}" target="_blank"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24">
                                            ...
                                            <path fill="red"
                                                d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4v2H5v14h14V9h2v10a2 2 0 0 1-2 2zm-9-4h2v-2H9v2zm-2-3h2v-2H7v2zm0-3h2v-2H7v2zm4 3h2v-2h-2v2zm0-3h2V7h-2v2zm4 3h2v-2h-2v2zm0-3h2V7h-2v2z" />
                                        </svg>
                                    </a>
                                @elseif ($download->gambar)
                                    <img src="{{ asset('storage/' . $download->gambar) }}" alt="Preview Gambar"
                                        class="rounded-lg shadow-md">
                                @else
                                    <span>No Preview Available</span>
                                @endif
                                <a href="{{ route('download.edit', $download->id) }}" data-tooltip-target="edit"
                                    class="cursor-pointer rounded-full border border-red-700/5 bg-red-700/5 p-3 text-red-700 transition-colors hover:border-red-700/10 hover:bg-red-700/10 hover:!opacity-100 group-hover:opacity-70">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('download.destroy', $download->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button data-tooltip-target="" type="button" data-id="{{ $download->id }}"
