...
@section('content')
    <section id="faq-index">
        <div class="container">
            ...
            <br>
            <button type="button" onclick="window.location='{{ route('kategori-berita.create') }}'"
                    ...
                    <tbody>
                        @foreach ($beritas as $berita)
                            <tr
                                class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="w-4 p-4"></td>
                                <td class="px-6 py-4 whitespace-nowrap dark:text-white">{{ $berita->judul }}</td>
                                <td class="px-6 py-4 whitespace-nowrap dark:text-white">{{ $berita->deskripsi_berita }}</td>
                                <td class="px-6 py-4 whitespace-nowrap dark:text-white">{{ $berita->kategori->nama_kategori }}</td>
                                <td class="px-6 py-4 whitespace-nowrap dark:text-white">
                                    <img src="{{$berita->gambar_berita }}" alt="Gambar Berita"
                                        class="h-12 w-12">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('berita.edit', ['id' => $berita->id]) }}"
                                        class="text-blue-600 hover:text-blue-900 dark:text-blue-500 dark:hover:text-blue-400">Edit</a>
                                    <form action="{{ route('berita.destroy', ['id' => $berita->id]) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-red-600 hover:text-red-900 dark:text-red-500 dark:hover:text-red-400">Remove</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
