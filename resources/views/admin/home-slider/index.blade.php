...
<section id="home-slider-index">
    <div class="container">
        <div class="my-6">
            <button type="button" onclick="window.location.href='{{ route('home-slider.create') }}'"
                    ...
                    @foreach($homeSliders as $homeSlider)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                            ...
                            <td class="flex items-center px-6 py-4">
                                ...
                                <form action="{{ route('homeslider.destroy', ['id' => $homeSlider->id]) }}" method="POST"
                                    class="delete-form inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" data-id="{{$homeSlider->id}}"
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function (event) {
                ...
                if (confirm('Are you sure you want to delete this download?')) {
                    ...
                    form.submit();
                    document.getElementById('homeSlider' + id).remove();
                }
            });
        });
    });
