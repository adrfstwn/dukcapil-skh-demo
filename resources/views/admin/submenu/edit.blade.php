        ...
        <div class="container">
            <div class="flex flex-col p-6 border-[1.5px] border-white shadow-md bg-slate-50 rounded-2xl shadow-gray-200">
                <div class="flex flex-col">
                    <h2 class="text-2xl font-bold md:text-3xl font-monserrats text-primary_teks">Edit Menu Section</h2>
                    <p class="text-base text-secondary_teks">Input for the Menu section</p>
                </div>
                <form action="{{ route('menu.update', ['id' => $menu->id]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                <div class="my-2">
                    <label for="default-input" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Nama Menu</label>
                    <input type="text" value="{{$menu->nama_menu}}" name="nama_menu" id="default-input"
                ...
                <div class="my-2">
                    ...
                    <input type="text" value="{{$menu->url}}" name="url" id="default-input"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary dark:focus:border-primary">
                </div>
                @if ($errors->any())
