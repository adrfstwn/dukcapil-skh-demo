                    ...
                    <div class="flex flex-col gap-4">
                        @foreach ($beritas as $berita)
                            <div class="flex flex-col gap-2">
                                <a href="{{ route('berita.detail', $berita->id) }}" class="font-bold font-nunito text-xl md:text-2xl text-primary_teks ">
                                    {{ $berita->judul }}
                                </a>
                                ...
                                <div class="flex flex-row items-center gap-2">
                                    @if ($berita->gambar_berita)
                                        ...
                                        <div class="flex flex-col"> <!-- Wrapped button in a div -->
                                            ...
                                            <a href="{{ route('berita.detail', $berita->id) }}" class="px-2 py-1 font-nunito text-xs text-background_light bg-primary rounded-sm mt-2" style="max-width: 150px;"> <!-- Adjusted padding and margin-top, and added inline styling to limit width -->
                                                Lihat Selengkapnya
                                            </a>
                                        </div>
                                    @endif
                ...
                <aside class="w-full md:w-1/3 md:pl-6">
                    ...
                    <div class="flex flex-col py-3 gap-4">
                        @foreach ($latestBeritas as $latestBerita)
                            <div class="flex flex-col gap-2">
                                <a href="{{ route('berita.detail', $latestBerita->id) }}" class="font-bold font-nunito text-xl md:text-2xl text-primary_teks ">
                                    {{ $latestBerita->judul }}
                                </a>
                                ...
                                <a href="{{ route('berita.detail', $latestBerita->id) }}" class="px-2 py-1 font-nunito text-xs text-background_light bg-primary rounded-sm mt-2" style="max-width: 150px;">
                                    Lihat Selengkapnya
                                </a>
                                <hr class="border-b-[1px] border-gray-300 mt-6 md:mt-8 rounded-full">
                            </div>
